...
class BaseTest : public ::testing::Test {
  protected:
    ...
    void SetUp() override {
      ...
      // todo: only run this onc time, instead of every time a test is run
      ...
      buildDir = std::filesystem::path(testBinaryDir).parent_path();
      ...
      // Set the stringstream as the target of the sink
      ...
      sbuf = std::cout.rdbuf();  // save cout buffer
      std::cout.rdbuf( cout_buffer.rdbuf() );  // redirect cout to buffer
    }
    ...
    std::filesystem::path buildDir;  // full directory of the build directory
...
class BuildWorkingDirectoryBase : public virtual BaseTest {
protected:

  void SetUp() override{
    std::cout << "BuildWorkingDirectoryBase:: starting Fixture SetUp" << std::endl;

    temp_dir = BaseTest::buildDir;
    std::cout << "BuildWorkingDirectoryBase:: temp_dir: " << temp_dir << std::endl;

    // change directory to `docs`
    original_dir = std::filesystem::current_path();  // save original directory
    std::cout << "BuildWorkingDirectoryBase:: original_dir: " << original_dir << std::endl;
    std::filesystem::current_path(temp_dir);
    std::cout << "BuildWorkingDirectoryBase:: working directory set to: " << std::filesystem::current_path() << std::endl;

    std::cout << "BuildWorkingDirectoryBase:: finished Fixture SetUp" << std::endl;
  }

  void TearDown() override{
    std::cout << "BuildWorkingDirectoryBase:: starting Fixture TearDown" << std::endl;

    // change directory back to original
    std::filesystem::current_path(original_dir);
    std::cout << "BuildWorkingDirectoryBase:: working directory set to: " << std::filesystem::current_path() << std::endl;

    std::cout << "BuildWorkingDirectoryBase:: finished Fixture TearDown" << std::endl;
  }
